...
use datafusion::physical_optimizer::pruning::{ColumnOrdering, PruningPredicate, PruningStatistics};
...
fn create_pruning_predicate(expr: Expr, schema: &SchemaRef) -> PruningPredicate {
    ...
    PruningPredicate::try_new(physical_expr, schema.clone(), &column_ordering).unwrap()
}
